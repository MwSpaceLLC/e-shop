...
export default withSession(
    async function getServerSideProps({query, req, locale}) {
        ...
        const
            product = (await axios.get(`http://${req.headers.host}/api/json/products/${productId}`)).data,
            category = (await axios.get(`http://${req.headers.host}/api/json/categories/${categoryId}`, fetcher)).data;
