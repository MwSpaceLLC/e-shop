    ...
    @foreach($category->product()->get() as $product)
        <tr data-id="{{$product->id}}" class="sortableItem">
            ...
            @if(eshop()->option('amazon'))
                <td data-tippy-content="{{isset($product->publish['amazon'])?'Pubblicato':'Non pubblicato'}}"
                    class="text-{{isset($product->publish['amazon'])?'success':'danger'}}"><i
                        class="fab fa-amazon"></i>
                </td>
            @endif
            @if(eshop()->option('amazon'))
                <td data-tippy-content="{{isset($product->publish['ebay'])?'Pubblicato':'Non pubblicato'}}"
                    class="text-{{isset($product->publish['ebay'])?'success':'danger'}}"><i
                        class="fab fa-ebay"></i>
                </td>
            @endif
            @if(eshop()->option('amazon'))
                <td data-tippy-content="{{isset($product->publish['merchant'])?'Pubblicato':'Non pubblicato'}}"
                    class="text-{{isset($product->publish['merchant'])?'success':'danger'}}"><i
                        class="fab fa-google"></i>
                </td>
            @endif
            @if(eshop()->option('amazon'))
                <td data-tippy-content="{{isset($product->publish['facebook'])?'Pubblicato':'Non pubblicato'}}"
                    class="text-{{isset($product->publish['facebook'])?'success':'danger'}}"><i
                        class="fab fa-facebook"></i>
                </td>
            @endif
            @if(eshop()->option('amazon'))
                <td data-tippy-content="{{isset($product->publish['alibaba'])?'Pubblicato':'Non pubblicato'}}">
                    <svg style="margin: unset" t="1578448004054" class="icon" viewBox="0 0 1041 1024" version="1.1"
                        ...
                        <path
                            ...
                            p-id="1137"
                            fill="#{{isset($product->publish['alibaba'])?'0F9D58':'FF788E'}}"></path>
                    </svg>
                </td>
            @endif
